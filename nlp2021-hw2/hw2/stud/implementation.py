...
def build_model_b(device: str) -> Model:
    ...
    """
    # return RandomBaseline(mode='b')
    return StudentModel(device=device, mode='b')
...
def build_model_ab(device: str) -> Model:
    ...
    return StudentModel(device=device, mode='ab')
...
def build_model_cd(device: str) -> Model:
    ...
    # raise NotImplementedError
    return StudentModel(device=device, mode='cd')
...
class StudentModel(Model):
    ...
    def __init__(self, device, mode = 'b'):
        ...
        self.model = ABSADB(mode=mode, device=device)
    ...
    def predict(self, samples: List[Dict]) -> List[Dict]:
        ...
        '''
        with torch.no_grad():
            results = self.model.eval().forward(samples,return_dict=True)
        return results
